...
const SideBar = ({
  ...
  return (
    <div className="relative flex flex-col md:fixed lg:w-1/5 md:w-[30%] h-screen mt-0 text-sm font-normal text-white bg-[#023d55]">
      <nav className="pl-4 mt-36">
        <div
          ...
          onClick={() => {
            gotoProfile ? router.push('/profile') : setEditProfile(true)
          }}
        >
          <div className="flex items-center justify-center w-8 h-8 bg-blue-500 rounded-l-lg">
            <FontAwesomeIcon
              icon={gotoProfile ? faUser : faPen}
              color="#FFFFFF"
              className="w-5"
            />
          </div>
          <p className="ml-2 font-bold text-black">
            {gotoProfile ? 'User Profile' : 'Edit Profile'}
          </p>
        </div>
        <div
